...
import { copyFileSync, existsSync, mkdirSync, writeFileSync } from 'node:fs' // Added writeFileSync
...
export async function createApp({
  ...
  if (example) {
    ...
    hasPackageJson = true // Templates always have a package.json
  }
  ...
  // ==========================================
  try {
    // 1. Automatically generate a .env.local file with boilerplates
    ...
    if (!existsSync(envPath)) {
      ...
      console.log(`Generated default ${cyan('.env.local')} file.`)
    }

    // 2. Automatically generate a build log metadata file
    const metaPath = join(root, 'project-meta.json')
    const metaData = {
      projectName: appName,
    ...
    console.warn('Warning: Failed to create custom configuration files.', customError)
  }
  // ==========================================
